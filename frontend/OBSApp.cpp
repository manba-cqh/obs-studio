...
static bool MakeUserDirs()
{
	...
	if (GetAppConfigPath(path, sizeof(path), "HuiXing/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetAppConfigPath(path, sizeof(path), "HuiXing/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetAppConfigPath(path, sizeof(path), "HuiXing/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetAppConfigPath(path, sizeof(path), "HuiXing/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif
	...
	if (GetAppConfigPath(path, sizeof(path), "HuiXing/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetAppConfigPath(path, sizeof(path), "HuiXing/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
constexpr std::string_view OBSProfileSubDirectory = "HuiXing/basic/profiles";
constexpr std::string_view OBSScenesSubDirectory = "HuiXing/basic/scenes";
constexpr std::string_view OBSPluginManagerSubDirectory = "HuiXing/plugin_manager";
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetAppConfigPath(path, sizeof(path), "HuiXing/global.ini");
	if (len <= 0) {
		return false;
	}
...
{
	const std::string userConfigFile = userConfigLocation.u8string() + "/HuiXing/user.ini";
...
static constexpr string_view OBSGlobalIniPath = "/HuiXing/global.ini";
static constexpr string_view OBSUserIniPath = "/HuiXing/user.ini";
...
bool LoadBranchesFile(vector<UpdateBranch> &out)
{
	...
	BPtr<char> branchesFilePath = GetAppConfigPathPtr("HuiXing/updates/branches.json");
...
static void move_basic_to_profiles(void)
{
	...
	if (GetAppConfigPath(path, 512, "HuiXing/basic") <= 0) {
		return;
	}
	...
	const std::filesystem::path profilesPath =
		App()->userProfilesLocation / std::filesystem::u8path("HuiXing/basic/profiles");
...
static void move_basic_to_scene_collections(void)
{
	...
	const std::filesystem::path sceneCollectionPath =
		App()->userScenesLocation / std::filesystem::u8path("HuiXing/basic/scenes");
...
void OBSApp::uploadLastAppLog() const
{
	...
	basicWindow->UploadLog("HuiXing/logs", GetLastLog(), OBS::LogFileType::LastAppLog);
}
...
void OBSApp::uploadCurrentAppLog() const
{
	...
	basicWindow->UploadLog("HuiXing/logs", GetCurrentLog(), OBS::LogFileType::CurrentAppLog);
}
